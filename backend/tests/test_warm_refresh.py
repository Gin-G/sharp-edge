...
get picked up — and that refresh must NOT re-persist the day's picks (persist is
insert-once; a changed slate would double-count the day).
"""
...
def test_aged_cache_triggers_background_refresh_without_persist(screen, monkeypatch):
    mod, captured = screen
    _set_cache(mod, monkeypatch, have_today=True, age=mod._WARM_TTL + 1)
    assert mod.warm_async() == {"status": "warming"}
    assert captured["started"] is True
    # An intra-day refresh must not re-record the day's picks.
    assert captured["kwargs"] == {"persist": False}


def test_cold_cache_warms_and_persists(screen, monkeypatch):
    mod, captured = screen
    _set_cache(mod, monkeypatch, have_today=False, age=None)
    assert mod.warm_async() == {"status": "warming"}
    assert captured["kwargs"] == {"persist": True}
